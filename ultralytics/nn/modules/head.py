...
class YOLOEDetect(Detect):
    ...
    def __init__(self, nc: int = 80, embed: int = 512, with_bn: bool = False, ch: Tuple = ()):
        ...
        # 添加cv6模块，用于提取BN处理后的512维特征
        # 重构cv6模块结构，使其完全复制cv3+cv4.bn的完整结构
        ...
        # 同步cv6模块的参数与cv3保持一致
        self._sync_cv6_with_cv3()

        self.reprta = Residual(SwiGLUFFN(embed, embed))
        self.savpe = SAVPE(ch, c3, embed)
        self.embed = embed

    def _sync_cv6_with_cv3(self):
        """同步cv6模块的参数与cv3保持一致"""
        try:
            for cv6_module, cv3_module in zip(self.cv6, self.cv3):
                # 同步前两层的参数
                for i in range(2):
                    # 同步DWConv层
                    if hasattr(cv6_module[i], '0') and hasattr(cv3_module[i], '0'):
                        cv6_dwconv = cv6_module[i][0]
                        cv3_dwconv = cv3_module[i][0]
                        if hasattr(cv6_dwconv, 'conv') and hasattr(cv3_dwconv, 'conv'):
                            cv6_dwconv.conv.weight.data.copy_(cv3_dwconv.conv.weight.data)
                            if (hasattr(cv3_dwconv, 'bn') and hasattr(cv6_dwconv, 'bn') and 
                                hasattr(cv3_dwconv.bn, 'weight') and hasattr(cv6_dwconv.bn, 'weight')):
                                cv6_dwconv.bn.weight.data.copy_(cv3_dwconv.bn.weight.data)
                                cv6_dwconv.bn.bias.data.copy_(cv3_dwconv.bn.bias.data)
                                cv6_dwconv.bn.running_mean.copy_(cv3_dwconv.bn.running_mean)
                                cv6_dwconv.bn.running_var.copy_(cv3_dwconv.bn.running_var)
                                # 复制其他BN参数
                                if hasattr(cv3_dwconv.bn, 'num_batches_tracked') and hasattr(cv6_dwconv.bn, 'num_batches_tracked'):
                                    cv6_dwconv.bn.num_batches_tracked.copy_(cv3_dwconv.bn.num_batches_tracked)

                    # 同步Conv层
                    if hasattr(cv6_module[i], '1') and hasattr(cv3_module[i], '1'):
                        cv6_conv = cv6_module[i][1]
                        cv3_conv = cv3_module[i][1]
                        if hasattr(cv6_conv, 'conv') and hasattr(cv3_conv, 'conv'):
                            cv6_conv.conv.weight.data.copy_(cv3_conv.conv.weight.data)
                            if (hasattr(cv3_conv, 'bn') and hasattr(cv6_conv, 'bn') and
                                hasattr(cv3_conv.bn, 'weight') and hasattr(cv6_conv.bn, 'weight')):
                                cv6_conv.bn.weight.data.copy_(cv3_conv.bn.weight.data)
                                cv6_conv.bn.bias.data.copy_(cv3_conv.bn.bias.data)
                                cv6_conv.bn.running_mean.copy_(cv3_conv.bn.running_mean)
                                cv6_conv.bn.running_var.copy_(cv3_conv.bn.running_var)
                                # 复制其他BN参数
                                if hasattr(cv3_conv.bn, 'num_batches_tracked') and hasattr(cv6_conv.bn, 'num_batches_tracked'):
                                    cv6_conv.bn.num_batches_tracked.copy_(cv3_conv.bn.num_batches_tracked)

                # 同步最后一层卷积的参数
                if hasattr(cv6_module, '2') and hasattr(cv3_module, '2'):
                    if (hasattr(cv6_module[2], 'weight') and hasattr(cv3_module[2], 'weight') and
                        cv6_module[2].weight.data.shape == cv3_module[2].weight.data.shape):
                        cv6_module[2].weight.data.copy_(cv3_module[2].weight.data)
                    if (hasattr(cv6_module[2], 'bias') and hasattr(cv3_module[2], 'bias') and
                        cv6_module[2].bias.data.shape == cv3_module[2].bias.data.shape):
                        cv6_module[2].bias.data.copy_(cv3_module[2].bias.data)
        except Exception as e:
            print(f"Warning: Error synchronizing cv6 with cv3: {e}")
            pass

        # # 同步cv6模块的BN参数与cv4中的一致
        # self._init_cv6_bn()

    def _init_cv6_bn(self):
        """初始化cv6中的BN层参数与cv4中的BN层一致"""
        try:
            for cv6_module, cv4_head in zip(self.cv6, self.cv4):
                if isinstance(cv4_head, BNContrastiveHead) and hasattr(cv4_head, 'norm'):
                    # 获取cv4中的BN参数
                    cv4_bn = cv4_head.norm
                    # cv6中的BN是最后一个模块
                    if len(cv6_module) > 3 and isinstance(cv6_module[-1], nn.BatchNorm2d):
                        cv6_bn = cv6_module[-1]
                        # 复制所有参数
                        if hasattr(cv4_bn, 'weight') and hasattr(cv6_bn, 'weight'):
                            cv6_bn.weight.data.copy_(cv4_bn.weight.data)
                        if hasattr(cv4_bn, 'bias') and hasattr(cv6_bn, 'bias'):
                            cv6_bn.bias.data.copy_(cv4_bn.bias.data)
                        if hasattr(cv4_bn, 'running_mean') and hasattr(cv6_bn, 'running_mean'):
                            cv6_bn.running_mean.copy_(cv4_bn.running_mean)
                        if hasattr(cv4_bn, 'running_var') and hasattr(cv6_bn, 'running_var'):
                            cv6_bn.running_var.copy_(cv4_bn.running_var)
                        # 复制eps和其他参数
                        if hasattr(cv4_bn, 'eps') and hasattr(cv6_bn, 'eps'):
                            cv6_bn.eps = cv4_bn.eps
                        if hasattr(cv4_bn, 'num_batches_tracked') and hasattr(cv6_bn, 'num_batches_tracked'):
                            cv6_bn.num_batches_tracked.copy_(cv4_bn.num_batches_tracked)
                        # 设置训练状态一致
                        cv6_bn.training = getattr(cv4_bn, 'training', False)
                        # 确保在eval模式下参数不会更新
                        if hasattr(cv4_bn, 'track_running_stats') and hasattr(cv6_bn, 'track_running_stats'):
                            cv6_bn.track_running_stats = cv4_bn.track_running_stats
        except Exception as e:
            print(f"Warning: Error initializing cv6 BN: {e}")
            pass

    def _sync_cv6_bn(self):
        """同步cv6中的BN参数与cv4保持一致"""
        try:
            for cv6_module, cv4_head in zip(self.cv6, self.cv4):
                if isinstance(cv4_head, BNContrastiveHead) and hasattr(cv4_head, 'norm'):
            ...
            print(f"Warning: Error synchronizing cv6 BN: {e}")
            pass

    def sync_cv6_weights(self):
        """
        在模型加载后手动同步cv6权重，确保cv6模块完全复制cv3+cv4.bn的参数
        这个方法应该在模型权重加载完成后调用
        """
        try:
            # 同步cv6模块的参数与cv3保持一致
            ...
            # 同步cv6模块的BN参数与cv4中的一致
            self._sync_cv6_bn()
            print("Successfully synchronized cv6 weights with cv3 and cv4.bn")
        except Exception as e:
            print(f"Warning: Error synchronizing cv6 weights: {e}")
            pass

    def get_features_before_fuse(self, x: List[torch.Tensor]) -> List[torch.Tensor]:
        """
        获取融合前的特征，用于TensorRT转换时保持特征一致性
        
        Args:
            x (List[torch.Tensor]): 输入特征图列表
            
        Returns:
            List[torch.Tensor]: cv6处理后的特征列表
        """
        features = []
        for i in range(self.nl):
            # 使用cv6模块处理特征，这等同于cv3+cv4.bn在融合前的处理
            feature = self.cv6[i](x[i])
            features.append(feature)
        return features

    def get_features_after_fuse(self, x: List[torch.Tensor]) -> List[torch.Tensor]:
        """
        获取融合后的特征，用于TensorRT转换时保持特征一致性
        
        Args:
            x (List[torch.Tensor]): 输入特征图列表
            
        Returns:
            List[torch.Tensor]: cv3处理后的特征列表（融合后）
        """
        features = []
        for i in range(self.nl):
            # 使用cv3模块处理特征，这是融合后的处理
            feature = self.cv3[i](x[i])
            features.append(feature)
        return features
